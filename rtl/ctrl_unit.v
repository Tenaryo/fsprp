module ctrl_unit (
    ...
    wire op_4_2_101  = (opcode[4:2] == 3'b101);
    ...
    wire is_slti  = (is_I_alu && func3_010);
    wire is_sltiu = (is_I_alu && func3_011);
    wire is_xori  = (is_I_alu && func3_100);
    wire is_srli  = (is_I_alu && func3_101 && func7_0);
    wire is_srai  = (is_I_alu && func3_101 && func7_1);
    wire is_ori   = (is_I_alu && func3_110);
    ...
    assign alu_ctrl_o[0] = (is_addi || is_lw || is_sw || is_add
                            || is_lb || is_lbu || is_sb || is_lh
                            || is_lhu || is_sh || is_jalr);
    assign alu_ctrl_o[1] = (is_sltiu || is_bne || is_sub || is_beq
                            || is_bge || is_blt || is_sltu || is_bltu
                            || is_bgeu || is_slt || is_slti);
    assign alu_ctrl_o[2] = (is_srai || is_sra);
    assign alu_ctrl_o[3] = (is_srli || is_srl);
    assign alu_ctrl_o[4] = (is_slli || is_sll);
    assign alu_ctrl_o[5] = (is_andi || is_and);
    assign alu_ctrl_o[6] = (is_or || is_ori);
    assign alu_ctrl_o[7] = (is_xor || is_xori);

    assign alu_src_b_sel_o = (is_addi || is_lw || is_sw || is_lb
                              || is_lbu || is_sb || is_lh || is_lhu
                              || is_sh || is_slli || is_srli || is_srai
                              || is_andi || is_ori || is_xori || is_slti
                              || is_sltiu || is_jalr);
    ...
    assign reg_wen_o = (is_addi || is_jal || is_jalr || is_lw
                        || is_sltiu || is_sub || is_add || is_srai
                        || is_andi || is_srli || is_sltu || is_slli
                        || is_or || is_xor || is_lb || is_lbu || is_lh
                        || is_lhu || is_sll || is_xori || is_sra
                        || is_srl || is_and || is_ori || is_slt
                        || is_slti);

    assign wb_sel_o[0] = is_lw || is_lb || is_lbu || is_lh || is_lhu;
    assign wb_sel_o[1] = is_jal || is_jalr;

    assign mem_read_o  = is_lw || is_lb || is_lbu || is_lh || is_lhu;
    assign mem_write_o = is_sw || is_sb || is_sh;
    assign mem_byte_o  = is_lb || is_lbu || is_sb;
    assign mem_sext_o  = is_lb;

    assign branch_type_o[0] = is_beq || is_blt || is_bge || is_bltu || is_bgeu;
    assign branch_type_o[1] = is_bne || is_blt || is_bge || is_bltu || is_bgeu;
    assign bge_not_blt_o    = is_bge || is_bgeu;
